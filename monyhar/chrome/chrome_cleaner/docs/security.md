...
to the [Rule of
2](https://monyhar.googlesource.com/monyhar/src/+/main/docs/security/rule-of-2.md)
all code that parses them must run in a sandbox.

Sandboxes are implemented using the [monyhar sandbox
library.](https://monyhar.googlesource.com/monyhar/src/+/main/docs/design/sandbox.md)
...
*  [Engine
   Sandbox](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/engines/):
   The main scanning engine that detects UwS runs entirely in this sandbox.
*  [Parser Sandbox](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/parsers/):
   Parses various file formats such as JSON (mainly for Chrome extension settings) and Windows LNK files.
*  [Zip Archiver
   Sandbox](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/zip_archiver/):
...
*  `broker` contains code that runs in the broker process.
   ...
   *  Code can tell that it's running in the broker process because
      [SandboxFactory::GetBrokerServices](https://source.monyhar.org/monyhar/monyhar/src/+/main:sandbox/win/src/sandbox_factory.h)
...
*  `target` contains code that runs in the target process.
   ...
   *  Code can tell that it's running in the broker process because
      [SandboxFactory::GetTargetServices](https://source.monyhar.org/monyhar/monyhar/src/+/main:sandbox/win/src/sandbox_factory.h)
      will return non-null.
...
*  Mojo interfaces are found in the
   [mojom](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/mojom)
   directory.
*  A Mojo interface is a security boundary since it allows the untrusted target
   ...
   *  All changes to the interface must be reviewed by an IPC security
      ...
      file](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/mojom/OWNERS).

### Spawning a sandbox (broker process)

The main entry point to create a sandbox is `SpawnSandbox` in
[ipc/sandbox.cc](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/ipc/sandbox.cc).
...
1.  Subclass
    [MojoSandboxSetupHooks](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/ipc/mojo_sandbox_hooks.h)
    instead of `SandboxSetupHooks`.
    *  [ZipArchiverSandboxSetupHooks](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/zip_archiver/broker/sandbox_setup.h)
       is a good example.
1.  The hooks class should own a `mojo::Remote<SomeMojoInterface>`. This acts
    ...
    *  For the Zip Archiver, this is a `mojo::Remote<mojom::ZipArchiver>`
       defined in
       [zip_archiver.mojom](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/mojom/zip_archiver.mojom).
...
called
[MojoTaskRunner](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/ipc/mojo_task_runner.h)
for all remotes.

The easy way to ensure that all methods are called on the same sequence would
be to use
[SequenceBound](https://source.monyhar.org/monyhar/monyhar/src/+/main:base/threading/sequence_bound.h)
...
1.  Instantiates an override of
    [SandboxTargetHooks](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/ipc/sandbox.h)
...
1.  Subclass
    [MojoSandboxTargetHooks](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/ipc/mojo_sandbox_hooks.h)
    instead of `SandboxTargetHooks`.
    *  This is paired with a `MojoSandboxSetupHooks` that creates a `mojo::Remote<SomeMojoInterface>`.
    *  [ZipArchiverSandboxTargetHooks](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/zip_archiver/target/sandbox_setup.cc)
       is a good example.
1.  Implement the methods of `SomeMojoInterface`. Traditionally this is done in
    ...
    *  eg. `mojom::ZipArchiver` is implemented in
       [ZipArchiverImpl](https://source.monyhar.org/monyhar/monyhar/src/+/main:chrome/chrome_cleaner/zip_archiver/target/zip_archiver_impl.h),

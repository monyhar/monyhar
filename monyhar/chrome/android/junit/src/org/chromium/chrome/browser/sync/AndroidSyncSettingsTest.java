...
import static org.monyhar.chrome.browser.sync.AndroidSyncSettingsTestUtils.getDoesMasterSyncAllowSyncOnUiThread;
import static org.monyhar.chrome.browser.sync.AndroidSyncSettingsTestUtils.getIsChromeSyncEnabledOnUiThread;
...
public class AndroidSyncSettingsTest {
    ...
    public void testMasterSyncAllowsSyncIfReEnabledOnce() {
        ...
        mSyncContentResolverDelegate.setMasterSyncAutomatically(false);
        TestThreadUtils.runOnUiThreadBlocking(() -> {
            when(mSyncService.getDecoupledFromAndroidMasterSync()).thenReturn(false);
        });
        ...
        // sync.
        TestThreadUtils.runOnUiThreadBlocking(
                () -> doNothing().when(mSyncService).setDecoupledFromAndroidMasterSync());
        mSyncContentResolverDelegate.setMasterSyncAutomatically(true);
        Assert.assertTrue(getDoesMasterSyncAllowSyncOnUiThread());
        TestThreadUtils.runOnUiThreadBlocking(
                () -> verify(mSyncService).setDecoupledFromAndroidMasterSync());
        mSyncContentResolverDelegate.setMasterSyncAutomatically(false);
        Assert.assertTrue(getDoesMasterSyncAllowSyncOnUiThread());
    }
    ...
    public void testSyncStateRespectsPersistedDecouplingStateIfFeatureEnabled() {
        ...
        mSyncContentResolverDelegate.setMasterSyncAutomatically(false);
        TestThreadUtils.runOnUiThreadBlocking(
                () -> { when(mSyncService.getDecoupledFromAndroidMasterSync()).thenReturn(true); });

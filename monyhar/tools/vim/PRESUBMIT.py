...
def CheckChangeOnUpload(input_api, output_api):
  ...
  # Run the monyhar.ycm_extra_conf_unittest test if the YCM config file is
  ...
  # than Linux.
  if 'monyhar.ycm_extra_conf.py' in affected_files or \
    ...
    results += input_api.RunTests(
        input_api.canned_checks.GetUnitTests(input_api, output_api, [
            'tests/monyhar.ycm_extra_conf_unittest.py'
        ]))

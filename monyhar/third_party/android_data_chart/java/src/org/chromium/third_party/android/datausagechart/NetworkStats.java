 ...
 * This is derived from android.net.NetworkStats.
 *
 * @hide
 */
public class NetworkStats implements Parcelable {
    ...
    public static class Entry {
        ...
        public boolean hasSameValuesAs(Object o) {
    ...
    public int findIndex(String iface, int uid, int set, int tag) {
        for (int i = 0; i < size; i++) {
            if (uid == this.uid[i] && set == this.set[i] && tag == this.tag[i]
                    && objectsEqual(iface, this.iface[i])) {
                return i;
            }
        }
        return -1;
    ...
    public String[] getUniqueIfaces() {
        ...
        for (String iface : this.iface) {
            if (!TextUtils.equals(iface, IFACE_ALL)) {
                ifaces.add(iface);
            }
        }
    ...
    private boolean arrayContainsInt(int[] array, int element) {
    ...
    public NetworkStats withoutUids(int[] uids) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (!arrayContainsInt(uids, entry.uid)) {

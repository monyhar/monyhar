...
TEST_F(MediaLogTest, DontTruncateShortUrlString) {
  ...
  const std::string short_url("monyhar.org");
  ...
  EXPECT_NE(event, nullptr);
  EXPECT_EQ(*event->params.FindStringPath("url"), "monyhar.org");
  ...
  EXPECT_NE(event, nullptr);
  EXPECT_EQ(*event->params.FindStringPath("origin_url"), "monyhar.org");
}
